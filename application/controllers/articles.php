...
class Articles extends Site_controller {
  ...
  public function index ($offset = 0) {
    ...
    return $this->set_title ($title . ' - ' . Cfg::setting ('site', 'title'))
                ...
                ->add_meta (array ('name' => 'description', 'content' => implode (' ', (array_map (function ($article) { return $article->title; }, array_merge (array ($title), $articles))))))
                ->add_meta (array ('property' => 'og:title', 'content' => $title . ' - ' . Cfg::setting ('site', 'title')))
                ->add_meta (array ('property' => 'og:description', 'content' => implode (' ', (array_map (function ($article) { return $article->title; }, array_merge (array ($title), $articles))))))
                ->load_view (array (
                    'articles' => $articles,
