...
class Dintaos extends Site_controller {
  ...
  public function index ($method, $index = Dintao::TYPE_OTHER, $offset = 0, $keyword = '') {
    ...
    if ($index > 0) Dintao::addConditions ($conditions, 'type = ?', $index);
    if ($keyword) Dintao::addConditions ($conditions, '(title LIKE ?) OR (content LIKE ?) OR (keywords LIKE ?)', '%' . $keyword . '%', '%' . $keyword . '%', '%' . $keyword . '%');
    ...
    $dintaos = Dintao::find ('all', array (
        'offset' => $offset,
        'limit' => $limit,
        'order' => $index > 0 ? 'sort DESC' : 'id DESC',
