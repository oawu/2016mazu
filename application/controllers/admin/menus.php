...
class Menus extends Admin_controller {
  ...
  public function update ($id = 0) {
    ...
    $old_roles = $menu->roles ();

    if ($add_roles = array_diff ($roles, $old_roles))
      foreach ($add_roles as $role)
        MenuRole::create (array ('menu_id' => $menu->id, 'role' => $role));

    if ($del_roles = array_diff ($old_roles, $roles))
      foreach (MenuRole::find ('all', array ('select' => 'id', 'conditions' => array ('menu_id = ? AND role IN (?)', $menu->id, $del_roles))) as $menu_role)
        $menu_role->destroy ();
  ...
  public function tree ($id = 0) {
    ...
    $roles = array_keys (Cfg::setting ('role'));
  ...
  private function _validation_posts (&$posts, $parent_menu) {
    ...
    if (isset ($posts['roles']) && $posts['roles'])
      if (!(($posts['roles'] = array_intersect ($posts['roles'], array_keys ($roles))) && (!$parent_menu || $parent_menu->roles () && ($posts['roles'] = array_intersect ($posts['roles'], $parent_menu->roles ())))))
        return '錯誤的角色！';
