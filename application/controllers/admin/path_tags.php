...
class Path_tags extends Admin_controller {
  private $uri_1 = null;
  private $tag = null;
  ...
  public function __construct () {
    ...
    $this->uri_1 = 'path-tags';

    if (in_array ($this->uri->rsegments (2, 0), array ('edit', 'update', 'destroy', 'sort')))
      if (!(($id = $this->uri->rsegments (3, 0)) && ($this->tag = PathTag::find_by_id ($id))))
  ...
  public function index ($offset = 0) {
    ...
    $limit = 25;
    $total = PathTag::count (array ('conditions' => $conditions));
  ...
  public function create () {
    ...
    $create = PathTag::transaction (function () use ($posts) {
      return verifyCreateOrm ($tag = PathTag::create (array_intersect_key ($posts, PathTag::table ()->columns)));
    });
  ...
  public function update () {
    ...
    $tag = $this->tag;
    $update = PathTag::transaction (function () use ($tag, $posts) {
      return $tag->save ();
    });
  ...
  public function destroy () {
    $tag = $this->tag;
    $delete = PathTag::transaction (function () use ($tag) {
      return $tag->destroy ();
    });
  ...
  public function is_on_site ($id = 0) {
    if (!($id && ($tag = PathTag::find_by_id ($id, array ('select' => 'id, is_on_site, updated_at')))))
      return $this->output_json (array ('status' => false, 'message' => '當案不存在，或者您的權限不夠喔！'));

    $posts = OAInput::post ();

    if ($msg = $this->_validation_is_on_site_posts ($posts))
      return $this->output_json (array ('status' => false, 'message' => $msg, 'content' => PathTag::$isOnSiteNames[$tag->is_on_site]));

    if ($columns = array_intersect_key ($posts, $tag->table ()->columns))
      foreach ($columns as $column => $value)
        $tag->$column = $value;

    $update = PathTag::transaction (function () use ($tag) { return $tag->save (); });

    if (!$update)
      return $this->output_json (array ('status' => false, 'message' => '更新失敗！', 'content' => PathTag::$isOnSiteNames[$tag->is_on_site]));

    return $this->output_json (array ('status' => true, 'message' => '更新成功！', 'content' => PathTag::$isOnSiteNames[$tag->is_on_site]));
  }
  ...
  private function _validation_posts (&$posts) {
    ...
    if (!(isset ($posts['is_on_site']) && is_numeric ($posts['is_on_site'] = trim ($posts['is_on_site'])) && in_array ($posts['is_on_site'], array_keys (PathTag::$isOnSiteNames))))
      $posts['is_on_site'] = PathTag::NO_ON_SITE_NAMES;

    return '';
  }
  private function _validation_is_on_site_posts (&$posts) {
    if (!(isset ($posts['is_on_site']) && is_numeric ($posts['is_on_site']) && in_array ($posts['is_on_site'], array_keys (PathTag::$isOnSiteNames))))
      return '參數錯誤！';
    return '';
  }
}
