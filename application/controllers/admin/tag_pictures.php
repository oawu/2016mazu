...
class Tag_pictures extends Admin_controller {
  ...
  public function __construct () {
    ...
    $this->tag_class = 'picture_tags';
    $this->uri_1     = 'tag';
    $this->uri_2     = 'pictures';

    if (!(($id = $this->uri->rsegments (3, 0)) && ($this->tag = PictureTag::find_by_id ($id))))
      return redirect_message (array ('admin', 'picture-tags'), array (
    ...
    $this->add_param ('class', $this->tag_class)
         ->add_tab ('標籤列表', array ('href' => base_url (array ('admin', 'picture-tags')), 'index' => 1))
         ->add_tab ($this->tag->name . '內的相簿列表', array ('href' => base_url ('admin', $this->uri_1, $this->tag->id, $this->uri_2), 'index' => 2))
         ->add_tab ('新增' . $this->tag->name . '的相簿', array ('href' => base_url ('admin', $this->uri_1, $this->tag->id, $this->uri_2, 'add'), 'index' => 3))
  ...
  public function index ($tag_id, $offset = 0) {
    ...
    return $this->set_tab_index (2)
                ->set_subtitle ($this->tag->name . '內的相簿列表')
  ...
  public function add ($tag_id) {
    ...
    return $this->set_tab_index (3)
                ->set_subtitle ('新增' . $this->tag->name . '的相簿')
                ->load_view (array (
                    'tags' => PictureTag::all (),
                    'posts' => $posts,
                  ));
