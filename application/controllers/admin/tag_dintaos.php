...
class Tag_dintaos extends Admin_controller {
  ...
  public function __construct () {
    ...
    $this->tag_class = 'dintao_tags';
    $this->uri_1     = 'tag';
    $this->uri_2     = 'dintaos';

    if (!(($id = $this->uri->rsegments (3, 0)) && ($this->tag = DintaoTag::find_by_id ($id))))
      return redirect_message (array ('admin', 'dintao-tags'), array (
    ...
    $this->add_param ('class', $this->tag_class)
         ->add_tab ('標籤列表', array ('href' => base_url (array ('admin', 'dintao-tags')), 'index' => 1))
         ->add_tab ($this->tag->name . '內的藝陣列表', array ('href' => base_url ('admin', $this->uri_1, $this->tag->id, $this->uri_2), 'index' => 2))
         ->add_tab ('新增' . $this->tag->name . '的藝陣', array ('href' => base_url ('admin', $this->uri_1, $this->tag->id, $this->uri_2, 'add'), 'index' => 3))
  ...
  public function index ($tag_id, $offset = 0) {
    ...
    return $this->set_tab_index (2)
                ->set_subtitle ($this->tag->name . '內的藝陣列表')
  ...
  public function add ($tag_id) {
    ...
    return $this->set_tab_index (3)
                ->set_subtitle ('新增' . $this->tag->name . '的藝陣')
                ->load_view (array (
                    'tags' => DintaoTag::all (),
                    'posts' => $posts,
                  ));
  ...
  public function edit () {
    ...
    return $this->add_tab ('編輯藝陣', array ('href' => base_url ('admin', $this->uri_1, $this->tag->id, $this->uri_2, $this->dintao->id, 'edit'), 'index' => 4))
                ->set_tab_index (4)
                ->set_subtitle ('編輯藝陣')
                ->load_view (array (
                    'posts' => $posts,
  ...
  public function update () {
    if (!$this->has_post ())
      return redirect_message (array ('admin', $this->uri_1, $this->tag->id, $this->uri_2, $this->dintao->id, 'edit'), array (
          '_flash_message' => '非 POST 方法，錯誤的頁面請求。'
        ));
