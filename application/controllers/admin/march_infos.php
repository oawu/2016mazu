...
class March_infos extends Admin_controller {
  private $uri_1 = null;
  private $info = null;
  private $march = null;
  ...
  public function __construct () {
    ...
    $this->uri_1 = 'march-infos';
    $this->march = March::find_by_id (1);

    if (in_array ($this->uri->rsegments (2, 0), array ('edit', 'update', 'destroy', 'sort')))
      if (!(($id = $this->uri->rsegments (3, 0)) && ($this->info = MarchInfo::find ('one', array ('conditions' => array ('id = ?', $id))))))
  ...
  public function index ($offset = 0) {
    ...
    $limit = 25;
    $total = MarchInfo::count (array ('conditions' => $conditions));
    ...
    $infos = MarchInfo::find ('all', array (
        'offset' => $offset,
        'limit' => $limit,
        'order' => 'id DESC',
  ...
  public function create () {
    ...
    $info = null;
    $create = MarchInfo::transaction (function () use (&$info, $posts) {
      if (!verifyCreateOrm ($info = MarchInfo::create (array_intersect_key ($posts, MarchInfo::table ()->columns))))
        return false;
      return true;
    });
  ...
  public function update () {
    ...
    $info = $this->info;
    $update = MarchInfo::transaction (function () use ($info, $posts) {
      if (!$info->save ())
        return false;
      return true;
    });
  ...
  public function destroy () {
    ...
    $info = $this->info;
    $delete = MarchInfo::transaction (function () use ($info) {
      return $info->destroy ();
    });
  ...
  private function _validation_posts (&$posts) {
    ...
    if (!$posts['messages'])
      return '沒有資訊，請填入至少一筆資訊！';
    $posts['msgs'] = json_encode (array_slice ($posts['messages'], 0));
    $posts['march_id'] = $this->march->id;
