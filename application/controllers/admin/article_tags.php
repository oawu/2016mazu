...
class Article_tags extends Admin_controller {
  ...
  public function is_on_site ($id = 0) {
    if (!($id && ($tag = ArticleTag::find_by_id ($id, array ('select' => 'id, is_on_site, updated_at')))))
      return $this->output_json (array ('status' => false, 'message' => '當案不存在，或者您的權限不夠喔！'));

    $posts = OAInput::post ();

    if ($msg = $this->_validation_is_on_site_posts ($posts))
      return $this->output_json (array ('status' => false, 'message' => $msg, 'content' => ArticleTag::$isOnSiteNames[$tag->is_on_site]));

    if ($columns = array_intersect_key ($posts, $tag->table ()->columns))
      foreach ($columns as $column => $value)
        $tag->$column = $value;

    $update = ArticleTag::transaction (function () use ($tag) { return $tag->save (); });

    if (!$update)
      return $this->output_json (array ('status' => false, 'message' => '更新失敗！', 'content' => ArticleTag::$isOnSiteNames[$tag->is_on_site]));

    return $this->output_json (array ('status' => true, 'message' => '更新成功！', 'content' => ArticleTag::$isOnSiteNames[$tag->is_on_site]));
  }
  ...
  private function _validation_posts (&$posts) {
    ...
    if (!(isset ($posts['is_on_site']) && is_numeric ($posts['is_on_site'] = trim ($posts['is_on_site'])) && in_array ($posts['is_on_site'], array_keys (ArticleTag::$isOnSiteNames))))
      $posts['is_on_site'] = ArticleTag::NO_ON_SITE_NAMES;

    return '';
  }
  private function _validation_is_on_site_posts (&$posts) {
    if (!(isset ($posts['is_on_site']) && is_numeric ($posts['is_on_site']) && in_array ($posts['is_on_site'], array_keys (ArticleTag::$isOnSiteNames))))
      return '參數錯誤！';
    return '';
  }
}
