...
class Store extends OaModel {
  ...
  static $has_many = array (
    ...
    array ('tags', 'class_name' => 'StoreTag', 'through' => 'mappings'),
    array ('sources', 'class_name' => 'StoreSource', 'order' => 'sort ASC')
  );
  ...
  public function __construct ($attributes = array (), $guard_attributes = true, $instantiating_via_find = false, $new_record = true) {
    ...
    OrmImageUploader::bind ('cover', 'StoreCoverImageUploader');
  }
  ...
  public function next ($tag_id = 0) {
    ...
    $is_loop = false;
    if (($tag_id && ($tag = StoreTag::find_by_id ($tag_id, array ('select' => 'id'))) && ($mapping = StoreTagMapping::find ('one', array ('conditions' => array ('store_id = ? AND store_tag_id = ?', $this->id, $tag->id)))) && (!(($store_ids = column_array (StoreTagMapping::find ('all', array ('select' => 'store_id', 'order' => 'store_id DESC', 'conditions' => array ('store_id != ? AND store_id <= ? AND store_tag_id = ?', $mapping->store_id, $mapping->store_id, $mapping->store_tag_id))), 'store_id')) && ($next = Store::find ('one', array ('order' => 'id DESC', 'conditions' => array ('id != ? AND id IN (?) AND destroy_user_id IS NULL AND is_enabled = ?', $this->id, $store_ids, self::IS_ENABLED)))))) && (!($is_loop && ($store_ids = column_array (StoreTagMapping::find ('all', array ('select' => 'store_id', 'conditions' => array ('store_id != ? AND store_tag_id = ?', $mapping->store_id, $mapping->store_tag_id))), 'store_id')) && ($next = Store::find ('one', array ('order' => 'id DESC', 'conditions' => array ('id != ? AND id IN (?) AND destroy_user_id IS NULL AND is_enabled = ?', $this->id, $store_ids, self::IS_ENABLED))))))) || (!($tag_id || ($next = Store::find ('one', array ('order' => 'id DESC', 'conditions' => array ('id != ? AND id <= ? AND destroy_user_id IS NULL AND is_enabled = ?', $this->id, $this->id, self::IS_ENABLED)))) || ($is_loop && ($next = Store::find ('one', array ('order' => 'id DESC', 'conditions' => array ('id != ? AND destroy_user_id IS NULL AND is_enabled = ?', $this->id, self::IS_ENABLED))))))))
      $next = null;
  ...
  public function prev ($tag_id = 0) {
    ...
    $is_loop = false;
    if (($tag_id && ($tag = StoreTag::find_by_id ($tag_id, array ('select' => 'id'))) && ($mapping = StoreTagMapping::find ('one', array ('conditions' => array ('store_id = ? AND store_tag_id = ?', $this->id, $tag->id)))) && (!(($store_ids = column_array (StoreTagMapping::find ('all', array ('select' => 'store_id', 'order' => 'store_id ASC', 'conditions' => array ('store_id != ? AND store_id >= ? AND store_tag_id = ?', $mapping->store_id, $mapping->store_id, $mapping->store_tag_id))), 'store_id')) && ($prev = Store::find ('one', array ('order' => 'id ASC', 'conditions' => array ('id != ? AND id IN (?) AND destroy_user_id IS NULL AND is_enabled = ?', $this->id, $store_ids, self::IS_ENABLED)))))) && (!($is_loop && ($store_ids = column_array (StoreTagMapping::find ('all', array ('select' => 'store_id', 'conditions' => array ('store_id != ? AND store_tag_id = ?', $mapping->store_id, $mapping->store_tag_id))), 'store_id')) && ($prev = Store::find ('one', array ('order' => 'id ASC', 'conditions' => array ('id != ? AND id IN (?) AND destroy_user_id IS NULL AND is_enabled = ?', $this->id, $store_ids, self::IS_ENABLED))))))) || (!($tag_id || ($prev = Store::find ('one', array ('order' => 'id ASC', 'conditions' => array ('id != ? AND id >= ? AND destroy_user_id IS NULL AND is_enabled = ?', $this->id, $this->id, self::IS_ENABLED)))) || ($is_loop && ($prev = Store::find ('one', array ('order' => 'id DESC', 'conditions' => array ('id != ? AND destroy_user_id IS NULL AND is_enabled = ?', $this->id, self::IS_ENABLED))))))))
      $prev = null;
  ...
  public function also ($tag_id = 0, $limit = 5) {
    ...
    if (!($tag_id && ($tag = StoreTag::find_by_id ($tag_id, array ('select' => 'id'))) && ($store_ids = column_array (StoreTagMapping::find ('all', array ('select' => 'store_id', 'limit' => $limit + 1, 'order' => 'store_id DESC', 'conditions' => array ('store_tag_id = ?', $tag->id))), 'store_id')) && ($also = Store::find ('all', array ('order' => 'id DESC', 'limit' => $limit, 'conditions' => array ('id != ? AND id IN (?) AND destroy_user_id IS NULL AND is_enabled = ?', $this->id, $store_ids, self::IS_ENABLED))))) && !($tag_id || ($also = Store::find ('all', array ('order' => 'id DESC', 'limit' => $limit, 'conditions' => array ('id != ? AND destroy_user_id IS NULL AND is_enabled = ?', $this->id, self::IS_ENABLED))))))
      $also = array ();
  ...
  public function content_page_url ($tag = null) {
    return $tag ? base_url ('tag', is_numeric ($tag) ? $tag : $tag->id, 'store', $this->content_page_last_uri ()) : base_url ('store', $this->content_page_last_uri ());
  }
