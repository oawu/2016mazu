...
class Article extends OaModel {
  ...
  public function next ($tag_id = 0) {
    ...
    $is_loop = false;
    if (($tag_id && ($tag = ArticleTag::find_by_id ($tag_id, array ('select' => 'id'))) && ($mapping = ArticleTagMapping::find ('one', array ('conditions' => array ('article_id = ? AND article_tag_id = ?', $this->id, $tag->id)))) && (!(($article_ids = column_array (ArticleTagMapping::find ('all', array ('select' => 'article_id', 'order' => 'article_id DESC', 'conditions' => array ('article_id != ? AND article_id <= ? AND article_tag_id = ?', $mapping->article_id, $mapping->article_id, $mapping->article_tag_id))), 'article_id')) && ($next = Article::find ('one', array ('order' => 'id DESC', 'conditions' => array ('id != ? AND id IN (?) AND destroy_user_id IS NULL AND is_enabled = ?', $this->id, $article_ids, self::IS_ENABLED)))))) && (!($is_loop && ($article_ids = column_array (ArticleTagMapping::find ('all', array ('select' => 'article_id', 'conditions' => array ('article_id != ? AND article_tag_id = ?', $mapping->article_id, $mapping->article_tag_id))), 'article_id')) && ($next = Article::find ('one', array ('order' => 'id DESC', 'conditions' => array ('id != ? AND id IN (?) AND destroy_user_id IS NULL AND is_enabled = ?', $this->id, $article_ids, self::IS_ENABLED))))))) || (!($tag_id || ($next = Article::find ('one', array ('order' => 'id DESC', 'conditions' => array ('id != ? AND id <= ? AND destroy_user_id IS NULL AND is_enabled = ?', $this->id, $this->id, self::IS_ENABLED)))) || ($is_loop && ($next = Article::find ('one', array ('order' => 'id DESC', 'conditions' => array ('id != ? AND destroy_user_id IS NULL AND is_enabled = ?', $this->id, self::IS_ENABLED))))))))
      $next = null;
  ...
  public function prev ($tag_id = 0) {
    ...
    $is_loop = false;
    if (($tag_id && ($tag = ArticleTag::find_by_id ($tag_id, array ('select' => 'id'))) && ($mapping = ArticleTagMapping::find ('one', array ('conditions' => array ('article_id = ? AND article_tag_id = ?', $this->id, $tag->id)))) && (!(($article_ids = column_array (ArticleTagMapping::find ('all', array ('select' => 'article_id', 'order' => 'article_id ASC', 'conditions' => array ('article_id != ? AND article_id >= ? AND article_tag_id = ?', $mapping->article_id, $mapping->article_id, $mapping->article_tag_id))), 'article_id')) && ($prev = Article::find ('one', array ('order' => 'id ASC', 'conditions' => array ('id != ? AND id IN (?) AND destroy_user_id IS NULL AND is_enabled = ?', $this->id, $article_ids, self::IS_ENABLED)))))) && (!($is_loop && ($article_ids = column_array (ArticleTagMapping::find ('all', array ('select' => 'article_id', 'conditions' => array ('article_id != ? AND article_tag_id = ?', $mapping->article_id, $mapping->article_tag_id))), 'article_id')) && ($prev = Article::find ('one', array ('order' => 'id ASC', 'conditions' => array ('id != ? AND id IN (?) AND destroy_user_id IS NULL AND is_enabled = ?', $this->id, $article_ids, self::IS_ENABLED))))))) || (!($tag_id || ($prev = Article::find ('one', array ('order' => 'id ASC', 'conditions' => array ('id != ? AND id >= ? AND destroy_user_id IS NULL AND is_enabled = ?', $this->id, $this->id, self::IS_ENABLED)))) || ($is_loop && ($prev = Article::find ('one', array ('order' => 'id DESC', 'conditions' => array ('id != ? AND destroy_user_id IS NULL AND is_enabled = ?', $this->id, self::IS_ENABLED))))))))
      $prev = null;
  ...
  public function also ($tag_id = 0, $limit = 5) {
    ...
    if (!($tag_id && ($tag = ArticleTag::find_by_id ($tag_id, array ('select' => 'id'))) && ($article_ids = column_array (ArticleTagMapping::find ('all', array ('select' => 'article_id', 'limit' => $limit + 1, 'order' => 'article_id DESC', 'conditions' => array ('article_tag_id = ?', $tag->id))), 'article_id')) && ($also = Article::find ('all', array ('order' => 'id DESC', 'limit' => $limit, 'conditions' => array ('id != ? AND id IN (?) AND destroy_user_id IS NULL AND is_enabled = ?', $this->id, $article_ids, self::IS_ENABLED))))) && !($tag_id || ($also = Article::find ('all', array ('order' => 'id DESC', 'limit' => $limit, 'conditions' => array ('id != ? AND destroy_user_id IS NULL AND is_enabled = ?', $this->id, self::IS_ENABLED))))))
      $also = array ();
  ...
  public function content_page_url ($tag = null) {
    return $tag ? base_url ('tag', is_numeric ($tag) ? $tag : $tag->id, 'article', $this->content_page_last_uri ()) : base_url ('article', $this->content_page_last_uri ());
  }
