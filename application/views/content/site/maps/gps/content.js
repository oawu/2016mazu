...
function circlePath (r) { return 'M 0 0 m -' + r + ', 0 '+ 'a ' + r + ',' + r + ' 0 1,0 ' + (r * 2) + ',0 ' + 'a ' + r + ',' + r + ' 0 1,0 -' + (r * 2) + ',0';}
  ...
  google.maps.event.addDomListener (window, 'load', function () {
    window.map = new google.maps.Map (window.$map.get (0), { zoom: 16, zoomControl: true, scrollwheel: true, scaleControl: true, mapTypeControl: false, navigationControl: true, streetViewControl: false, disableDoubleClickZoom: true, center: new google.maps.LatLng (23.569396231491233, 120.3030703338623)});
    ...
    //   map: window.map, radius: 20, opacity: 0.9,
    //   data: window.$map.data ('p').map (function (t) {
    //     return new google.maps.LatLng (t.a, t.n);
    //   })
    // });

    // window.$map.data ('p').forEach (function (t) {
    ...
    //       strokeWeight: 1,
    //       fillColor: 'rgba(249, 39, 114, 1)',
    //       fillOpacity: 1
    //     }
    //   });
    //   // return new google.maps.LatLng (t.a, t.n);
    // });
    var polyline = new google.maps.Polyline ({ map: window.map, strokeColor: 'rgba(249, 39, 114, .45)', strokeWeight: 5, path: window.$map.data ('p').map (function (t) {
    // // new google.maps.Marker ({ map: _map, draggable: false, optimized: false});
      return new google.maps.LatLng (t.a, t.n);
    })});
