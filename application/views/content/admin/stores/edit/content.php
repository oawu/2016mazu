...
<?php
  if ((isset ($posts['latitude']) && ($latitude = $posts['latitude'])) || ($latitude = $store->latitude)) { ?>
    <input type='hidden' class='ori_latitude' value='<?php echo $latitude;?>'/>
<?php
  }
  if ((isset ($posts['longitude']) && ($longitude = $posts['longitude'])) || ($longitude = $store->longitude)) { ?>
  ...
  <table class='table-form'>
    <tbody>
      ...
      <tr>
        <th>標 題：</th>
        <td>
          <input type='text' name='title' value='<?php echo isset ($posts['title']) ? $posts['title'] : $store->title;?>' placeholder='請輸入標題..' maxlength='200' pattern='.{1,200}' required title='輸入標題!' autofocus />
        </td>
      </tr>
      ...
      <tr>
        <th>封 面：</th>
        <td>
          <?php echo (string)$store->cover ? img ($store->cover->url ('100x100c'), false, 'class="cover"') : '';?>
      ...
      <tr>
        <th>關鍵字：</th>
        <td class='k'>
          <input type='text' name='keywords' value='<?php echo isset ($posts['keywords']) ? $posts['keywords'] : $store->keywords;?>' placeholder='請輸入關鍵字..' maxlength='200' pattern='.{1,200}' required title='輸入關鍵字!' />
...
<?php if ($tags) { ?>
        <tr>
          <th>標 籤：</th>
          <td>
      <?php $tag_ids = isset ($posts['tag_ids']) ? $posts['tag_ids'] : column_array ($store->mappings, 'store_tag_id');
            foreach ($tags as $tag) { ?>
...
<?php }?>
      ...
      <tr>
        <th>內 容：</th>
        <td>
          <textarea name='content' class='cke' placeholder='請輸入描述..'><?php echo isset ($posts['content']) ? $posts['content'] : $store->content;?></textarea>
        </td>
      </tr>
      ...
      <tr>
        ...
        <?php foreach (Store::icon_urls () as $i => $url) { ?>
                <label for='type_<?php echo $i;?>'>
                  <input type='radio' id='type_<?php echo $i;?>' name='type' value='<?php echo $i;?>'<?php echo isset ($posts['type']) ? $posts['type'] == $i : ($store->type == $i) ? ' checked' : '';?>/>
