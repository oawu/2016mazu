...
<?php if ($stores) {
        foreach ($stores as $store) { ?>
          <tr>
            <td data-title='封面' width='50'><?php echo img ($store->cover->url ('100x100c'), false, 'class="i_30"');?></td>
            <td data-title='標題' width='150'><?php echo $store->title;?></td>
            <td data-title='描述' width='' class='left'><?php echo $store->mini_content ();?></td>
            <td data-title='關鍵字' width='200' class='left'><?php echo $store->mini_keywords ();?></td>
            <td data-title='標籤' width='150'><?php echo $store->tags ? implode ('<br/>', array_map (function ($tag) { return anchor (base_url ('admin', 'tag', $tag->id, 'stores'), $tag->name, 'class="tag"'); }, $store->tags)) : '-';?></td>
            <td data-title='PV' width='80'><?php echo $store->pv;?></td>
            <td data-title='是否啟用' width='90'>
              <label class='index_checkbox'>
                <input type='checkbox' data-id='<?php echo $store->id;?>'<?php echo $store->is_enabled == Store::IS_ENABLED ? ' checked' : '';?>>
                <span></span><div><?php echo Store::$isIsEnabledNames[$store->is_enabled];?></div>
              </label>
            </td>
