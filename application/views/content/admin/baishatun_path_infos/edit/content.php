...
<?php
  if ((isset ($posts['lat']) && ($lat = $posts['lat'])) || ($lat = $info->lat)) { ?>
    <input type='hidden' class='ori_lat' value='<?php echo $lat;?>'/>
<?php
  }
  if ((isset ($posts['lng']) && ($lng = $posts['lng'])) || ($lng = $info->lng)) { ?>
    <input type='hidden' class='ori_lng' value='<?php echo $lng;?>'/>
<?php
  }
  if ($points) {
