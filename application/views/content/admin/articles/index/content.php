...
<?php if ($articles) {
        foreach ($articles as $article) { ?>
          <tr>
            ...
            <td data-title='標籤' width='150'><?php echo $article->tags ? implode ('<br/>', array_map (function ($tag) { return anchor (base_url ('admin', 'tag', $tag->id, 'articles'), $tag->name, 'class="tag"'); }, $article->tags)) : '-';?></td>
            ...
            <td data-title='編輯' width='80'>
              <a href='<?php echo base_url ('admin', $uri_1, $article->id, 'edit');?>' class='icon-pencil2'></a>
              <a href='<?php echo base_url ('admin', $uri_1, $article->id);?>' data-method='delete' class='icon-bin destroy'></a>
            </td>
          </tr>
  <?php }
      } else { ?>
