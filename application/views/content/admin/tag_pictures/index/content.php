...
<?php if ($pictures) {
        foreach ($pictures as $picture) { ?>
          <tr>
            <td data-title='封面' width='50'><?php echo img ($picture->name->url ('100x100c'), false, 'class="i_30"');?></td>
            ...
            <td data-title='是否啟用' width='90'>
              <label class='index_checkbox'>
                <input type='checkbox' data-id='<?php echo $picture->id;?>'<?php echo $picture->is_enabled == Picture::IS_ENABLED ? ' checked' : '';?>>
                <span></span><div><?php echo Picture::$isIsEnabledNames[$picture->is_enabled];?></div>
              </label>
            </td>
            <td data-title='編輯' width='80'>
              <a href='<?php echo base_url ('admin', $uri_1, $tag->id, $uri_2, $picture->id, 'edit');?>' class='icon-pencil2'></a>
              <a href='<?php echo base_url ('admin', $uri_1, $tag->id, $uri_2, $picture->id);?>' data-method='delete' class='icon-bin destroy'></a>
            </td>
          </tr>
  <?php }
      } else { ?>
