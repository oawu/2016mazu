...
<?php if ($articles) {
        foreach ($articles as $article) { ?>
          <tr>
            <td data-title='封面' width='50'><?php echo img ($article->cover->url ('100x100c'), false, 'class="i_30"');?></td>
            <td data-title='標題' width='150'><?php echo $article->title;?></td>
            <td data-title='描述' width='' class='left'><?php echo $article->mini_content ();?></td>
            <td data-title='關鍵字' width='200' class='left'><?php echo $article->mini_keywords ();?></td>
            <td data-title='標籤' width='150'><?php echo $article->tags ? implode ('<br/>', array_map (function ($tag) { return anchor (base_url ('admin', 'article-tag', $tag->id, 'articles'), $tag->name, 'class="tag"'); }, $article->tags)) : '-';?></td>
            <td data-title='PV' width='80'><?php echo $article->pv;?></td>
            <td data-title='是否啟用' width='90'>
              <label class='index_checkbox'>
                <input type='checkbox' data-id='<?php echo $article->id;?>'<?php echo $article->is_enabled == Article::IS_ENABLED ? ' checked' : '';?>>
                <span></span><div><?php echo Article::$isIsEnabledNames[$article->is_enabled];?></div>
              </label>
            </td>
            <td data-title='編輯' width='80'>
              <a href='<?php echo base_url ('admin', $uri_1, $tag->id, $uri_2, $article->id, 'edit');?>' class='icon-pencil2'></a>
              <a href='<?php echo base_url ('admin', $uri_1, $tag->id, $uri_2, $article->id);?>' data-method='delete' class='icon-bin destroy'></a>
            </td>
          </tr>
  <?php }
      } else { ?>
