<form action='<?php echo base_url (array ('admin', $uri_1));?>' method='post' enctype='multipart/form-data'>
  <table class='table-form'>
    <tbody>
      ...
      <?php if ($isOnSiteNames = YoutubeTag::$isOnSiteNames) {
              foreach ($isOnSiteNames as $key => $name) { ?>
                <option value='<?php echo $key;?>'<?php echo (isset ($posts['is_on_site']) ? $posts['is_on_site'] : YoutubeTag::NO_ON_SITE_NAMES) == $key ? ' selected': '';?>><?php echo $name;?></option>
        <?php }
            }?>
          </select>
        </td>
