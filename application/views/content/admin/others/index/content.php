...
<?php if ($others) {
        foreach ($others as $other) { ?>
          <tr>
            <td data-title='封面' width='50'><?php echo img ($other->cover->url ('100x100c'), false, 'class="i_30"');?></td>
            <td data-title='標題' width='150'><?php echo $other->title;?></td>
            <td data-title='描述' width='' class='left'><?php echo $other->mini_content ();?></td>
            <td data-title='關鍵字' width='140' class='left'><?php echo $other->mini_keywords ();?></td>
            <td data-title='PV' width='80'><?php echo $other->pv;?></td>
            <td data-title='是否啟用' width='90'>
              <label class='index_checkbox'>
                <input type='checkbox' data-id='<?php echo $other->id;?>'<?php echo $other->is_enabled == Other::IS_ENABLED ? ' checked' : '';?>>
                <span></span><div><?php echo Other::$isIsEnabledNames[$other->is_enabled];?></div>
              </label>
            </td>
            <td data-title='編輯' width='80'>
              <a href='<?php echo base_url ('admin', $uri_1, $other->id, 'edit');?>' class='icon-pencil2'></a>
              <a href='<?php echo base_url ('admin', $uri_1, $other->id);?>' data-method='delete' class='icon-bin destroy'></a>
            </td>
          </tr>
  <?php }
      } else { ?>
