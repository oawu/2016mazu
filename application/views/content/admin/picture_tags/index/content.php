<form action='<?php echo base_url ('admin', 'picture_tags');?>' method='get' class="search<?php echo $has_search ? ' show' : '';?>">
  ...
  <a href='<?php echo base_url ('admin', 'picture_tags', 'add');?>'>新增</a>
</form>
        ...
        foreach ($tags as $tag) { ?>
          <tr>
            ...
            <td data-title='編輯' width='120'>
              ...
              <a href='<?php echo base_url ('admin', 'picture_tags', $tag->id);?>' data-method='delete' class='icon-bin destroy'></a>
            </td>
            <td data-title='排序' width='60' class='sort'>
              <a href='<?php echo base_url ('admin', 'picture_tags', $tag->id, 'sort', 'up');?>' data-method='post' class='icon-triangle-up'></a>
              <a href='<?php echo base_url ('admin', 'picture_tags', $tag->id, 'sort', 'down');?>' data-method='post' class='icon-triangle-down'></a>
            </td>
          </tr>
  <?php }
      } else { ?>
