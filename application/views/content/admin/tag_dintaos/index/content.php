...
<?php if ($dintaos) {
        foreach ($dintaos as $dintao) { ?>
          <tr>
            ...
            <td data-title='標籤' width='150'><?php echo $dintao->tags ? implode ('<br/>', array_map (function ($tag) { return anchor (base_url ('admin', 'dintao-tag', $tag->id, 'dintaos'), $tag->name, 'class="tag"'); }, $dintao->tags)) : '-';?></td>
            ...
            <td data-title='編輯' width='80'>
              <a href='<?php echo base_url ('admin', $uri_1, $tag->id, $uri_2, $dintao->id, 'edit');?>' class='icon-pencil2'></a>
              <a href='<?php echo base_url ('admin', $uri_1, $tag->id, $uri_2, $dintao->id);?>' data-method='delete' class='icon-bin destroy'></a>
            </td>
          </tr>
  <?php }
      } else { ?>
