<nav>
  <div class='container'>
    ...
    <?php } else { ?>
              <a href='<?php echo Fb::logoutUrl ('platform', 'sign_out');?>' class='top_line icon-menu'>登出</a>
        <?php if (in_array ('admin', User::current ()->role_names ())) { ?>
                <a class='icon-menu' href='<?php echo base_url ('admin');?>'>管理</a>
        <?php }?>
    <?php }?>
        </div>
      </div>
