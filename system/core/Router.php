...
class Route {
	...
	public static function resourcePagination_site ($uris, $controller, $prefix = '') {
		$uris = is_string ($uris) ? array ($uris) : $uris;

		if (!is_array ($last = array_pop ($uris))) $last = array ($last);
		if ($last) array_push ($uris, $last[0]);
		...
		if ($last && (count ($last) > 1)) {
			$uris1 = array_merge ($uris, array ($last[0]));
			$uris2 = array_merge ($uris, array ($last[1]));

			Route::get ($prefix . implode ('/(:id)/', $uris1) . '/(:id)-', $prefix . $controller . '@show($1' . ($c > 1 ? ', ' . implode (', ', array_map (function ($a) { return '$' . $a; }, range (2, $c))) : '') . ')');
			Route::get ($prefix . implode ('/(:id)/', $uris1) . '/(:id)-(:any)', $prefix . $controller . '@show($1' . ($c > 1 ? ', ' . implode (', ', array_map (function ($a) { return '$' . $a; }, range (2, $c))) : '') . ')');

			Route::get ($prefix . implode ('/(:id)/', $uris2) . '/', $prefix . $controller . '@index(0,' . ($c > 1 ? implode (', ', array_map (function ($a) { return '$' . $a; }, range (1, $c - 1))) . ', ' : '') . ')');
			Route::get ($prefix . implode ('/(:id)/', $uris2) . '/(:id)', $prefix . $controller . '@show($1' . ($c > 1 ? ', ' . implode (', ', array_map (function ($a) { return '$' . $a; }, range (2, $c))) : '') . ')');
			Route::get ($prefix . implode ('/(:id)/', $uris2) . '/(:id)-', $prefix . $controller . '@show($1' . ($c > 1 ? ', ' . implode (', ', array_map (function ($a) { return '$' . $a; }, range (2, $c))) : '') . ')');
			Route::get ($prefix . implode ('/(:id)/', $uris2) . '/(:id)-(:any)', $prefix . $controller . '@show($1' . ($c > 1 ? ', ' . implode (', ', array_map (function ($a) { return '$' . $a; }, range (2, $c))) : '') . ')');
		}
	}
	...
	public static function resourcePagination_is_on_site ($uris, $controller, $prefix = '') {
		$c = count ($uris = is_string ($uris) ? array ($uris) : $uris);
		$group = self::_getGroup ();
		$prefix = trim ($group . trim ($prefix, '/'), '/') . '/';
		...
		self::put ($prefix . implode ('/(:id)/', $uris) . '/(:id)', $prefix . $controller . '@update($1' . ($c > 1 ? ', ' . implode (', ', array_map (function ($a) { return '$' . $a; }, range (2, $c))) : '') . ')');
		self::delete ($prefix . implode ('/(:id)/', $uris) . '/(:id)', $prefix . $controller . '@destroy($1' . ($c > 1 ? ', ' . implode (', ', array_map (function ($a) { return '$' . $a; }, range (2, $c))) : '') . ')');
		self::post ($prefix . implode ('/(:id)/', $uris) .  '/is_on_site/(:id)', $prefix . $controller . '@is_on_site($1' . ($c > 1 ? ', ' . implode (', ', array_map (function ($a) { return '$' . $a; }, range (2, $c))) : '') . ')');
	}
